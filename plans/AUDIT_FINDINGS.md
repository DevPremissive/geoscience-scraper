...
\n- **2026-08-21 (C1.6/C6.5/C6.3)** — section Q: Ontario publishes no banked credit balances, so C6.5 reports the runway as unavailable and keeps historical work value as a separate, non-additive quantity (Q1); C1.6's confirmation needs the claim to be GONE from the register, because 12,116 are past due and still held (Q2); and C6.3's entry-window flag is ANTI-predictive — 33% on rush belts against 66% on random controls — while its acceptance criterion passes it anyway, because 'flag at least one belt' over a large search space has no base rate in it (Q3).\n